class Guard
  ...
  def minutes_slept
    @nights.values.flatten.sum
  end

  def minute_most_slept
    @nights.values.transpose.map { |a| a.sum }.each_with_index.max.last
  end
end
...
sleepiest_guard = gl.values.max_by { |guard| guard.minutes_slept }
p sleepiest_guard.id * sleepiest_guard.minute_most_slept
